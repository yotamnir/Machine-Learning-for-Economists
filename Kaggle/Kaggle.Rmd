...
pacman::p_load(tidyverse, tidymodels, kableExtra, glmnet, ranger, vip, xgboost)
...
Here I split the sample 80/20 to train and test, as a first step for my own reference (I should use the full training set when submitting my estimations), and split into five folds.

I also experiment with some features which may improve predictive ability.
...
The out of sample MSE is quite similar to that obtained using regularized regressions.
...
```
Following several iterations, it appears that boosting does slightly better than the previous methods on average, but there doesn't appear to be any significant difference.
